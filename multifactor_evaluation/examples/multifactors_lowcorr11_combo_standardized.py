...
DEFAULT_OUTPUT_DIR = Path("/Volumes/Quant_Disk/evaluation/multifactors_lowcorr11_combo_standardized")
...
def _load_low_corr_names(path: Path) -> list[str]:
    if not path.exists():
...
def _build_combined_panel(factor_panels: dict[str, pd.DataFrame]) -> tuple[pd.DataFrame, list[str]]:
    ...
    count_panel = pd.DataFrame(contribution_count, index=common_index, columns=common_columns)
    return combined_panel, list(aligned.keys()), count_panel
...
def main() -> None:
    ...
    selected_factors = _load_low_corr_names(low_corr_table)[:DEFAULT_TOP_K]
    ...
    combined_factor, selected_order, contribution_count = _build_combined_panel(factor_panels)
    ...
    summary = {
        ...
        "contribution_mean": float(contribution_count.stack().mean()),
        "contribution_median": float(contribution_count.stack().median()),
        "backtest_stats": stats,
        "output_dir": str(output_dir),
    }

    combined_factor.to_parquet(output_dir / "lowcorr11_combo_standardized_factor.parquet")
    lagged_combined.to_parquet(output_dir / "lowcorr11_combo_standardized_factor_lagged.parquet")
    contribution_count.to_parquet(output_dir / "lowcorr11_combo_standardized_contribution_count.parquet")
    backtest.to_parquet(output_dir / "lowcorr11_combo_standardized_backtest_daily.parquet")
    pd.DataFrame(stats).T.to_parquet(output_dir / "lowcorr11_combo_standardized_backtest_stats.parquet")
    pd.DataFrame(list(thresholds.items()), columns=["factor_name", "threshold"]).to_parquet(
        output_dir / "lowcorr11_combo_standardized_thresholds.parquet",
        index=False,
    )
    (output_dir / "lowcorr11_combo_standardized_summary.json").write_text(
        json.dumps(summary, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )

    print("Low-correlation 11-factor standardized combo strategy complete")
