...
DEFAULT_OUTPUT_DIR = Path("/Volumes/Quant_Disk/evaluation/multifactors_lowcorr_combo")
DEFAULT_REPRESENTATIVE_FACTOR = "alpha_047"
DEFAULT_LOW_CORR_COUNT = 5
DEFAULT_THRESHOLD_MULTIPLIER = 0.75
...
def _factor_scale(frame: pd.DataFrame) -> float:
    row_std = frame.std(axis=1, ddof=0).replace([np.inf, -np.inf], np.nan).dropna()
    if row_std.empty:
        return 1.0
    median = float(row_std.median())
...
def _build_combined_panel(
    ...
    *,
    ...
    for name, panel in aligned.items():
        scale = _factor_scale(panel)
        threshold = float(thresholds.get(name, DEFAULT_THRESHOLD_MULTIPLIER)) * scale
        zscore = panel.where(panel.abs() >= threshold)
    ...
    return combined_panel, list(aligned.keys())
...
def main() -> None:
    ...
    thresholds = {name: DEFAULT_THRESHOLD_MULTIPLIER for name in selected_factors}
    ...
    combined_factor, selected_order = _build_combined_panel(factor_panels, thresholds=thresholds)
    ...
    combined_factor.to_parquet(output_dir / "lowcorr_combo_factor.parquet")
    lagged_combined.to_parquet(output_dir / "lowcorr_combo_factor_lagged.parquet")
    backtest.to_parquet(output_dir / "lowcorr_combo_backtest_daily.parquet")
    pd.DataFrame(stats).T.to_parquet(output_dir / "lowcorr_combo_backtest_stats.parquet")
    pd.Series(thresholds, name="threshold").to_frame().to_parquet(output_dir / "lowcorr_combo_thresholds.parquet")
    (output_dir / "lowcorr_combo_summary.json").write_text(
        json.dumps(summary, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )

    print("Low-correlation combo strategy complete")
