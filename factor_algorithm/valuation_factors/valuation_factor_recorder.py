...
DEFAULT_OUTPUT_ROOT = Path("/Volumes/Quant_Disk/evaluation/valuation_factors")
    ...
    *,
    ...
    for engine_cls in (QuarterlyFinancialFactorEngine, AnnualFinancialFactorEngine, TTMFinancialFactorEngine):
        for variant_name, variant in variant_map.items():
            ...
            factor_frame = engine.compute_valuation_factor_frame(max_factors=max_factors)
            bundle = engine.save_valuation_factor_files(
                name=f"{engine.output_tag}_{variant_name}_valuation_factors",
...
def main() -> list[dict[str, object]]:
    parser = argparse.ArgumentParser(description="Record SimFin valuation factors.")
    ...
    return record_valuation_factors(
        start=args.start,
        end=args.end,
