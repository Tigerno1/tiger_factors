...
def build_code_industry_frame(companies: object, industries: object) -> pd.DataFrame:
    """Build a code -> industry lookup frame from SimFin company and industry tables."""
    companies_df = _as_pandas_frame(companies)
    industries_df = _as_pandas_frame(industries)

    companies_df.columns = [str(column).lower() for column in companies_df.columns]
    industries_df.columns = [str(column).lower() for column in industries_df.columns]

    code_column = "code" if "code" in companies_df.columns else "ticker" if "ticker" in companies_df.columns else None
    if code_column is None:
        raise KeyError("companies is missing a code/ticker column")
    if "industry_id" not in companies_df.columns:
        raise KeyError("companies is missing industry_id")
    if "industry_id" not in industries_df.columns:
        raise KeyError("industries is missing industry_id")

    join_columns = ["industry_id"] + [column for column in ("industry", "sector") if column in industries_df.columns]

    result = (
        companies_df[[code_column, "industry_id"]]
        .rename(columns={code_column: "code"})
        .drop_duplicates(subset=["code"])
        .merge(
            industries_df[join_columns].drop_duplicates(subset=["industry_id"]),
            on="industry_id",
            how="left",
        )
        .drop_duplicates(subset=["code"])
...
class Alpha101Engine:
    ...
    def _prepare_input(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing:
            raise ValueError(f"alpha101 input is missing columns: {sorted(missing)}")
...
def alpha101_factor_names() -> list[str]:
    return [f"alpha_{i:03d}" for i in range(1, 102)]


if __name__ == "__main__":
   
    from tiger_api.sdk import fetch_codes, fetch_data
    ...
    # codes = fetch_codes(**request, at="2021-06-01")
    # print(codes)
    ...
    # prices = fetch_data(**request, codes=codes, at="2021-06-01", return_type="pl")
    # # update_data(**request)
    # print(prices)
